...
We used the median shape of the combined experts' data as a baseline for all comparisons, as the "true" shape. To assess within-individual variation, we compared each worker's shape replicate to their own median shape. To assess within-group and between-group variation, we also compare those individual worker median shapes to the median shape of all other turkers and all other experts. We also measured the median absolute deviation (MAD) as a dispersion statistic for within-worker, within-group, and between-group comparisons.

To test whether turkers and experts were statistically distinguishable, 
...
```{r sum_table}
no_st <- fish_landmarks %>% filter(role != "student") %>% mutate(role = factor(role))

pcaed <- no_st %>% group_by(family, mark) %>% do(as_df(prcomp(cbind(.$x, .$y), scale=F, center=T)$x, role=as.character(.$role), ident=as.character(.$sha1mac))) %>% mutate(PC1 = as.numeric(PC1), PC2 = as.numeric(PC2))

meds <- pcaed %>% group_by(family, role, mark) %>% summarise(med=median(PC1)) %>% ungroup() %>% spread(role, med) %>% transmute(family, mark, value = log(abs(turker - morphologist)), stat="median")

mads <- pcaed %>% group_by(family, role, mark) %>% summarise(mad=mad(PC1)) %>% ungroup() %>% spread(role, mad) %>% transmute(family, mark, value = log(turker / morphologist), stat="mad")

all_stats <- bind_rows(meds, mads) %>% spread(stat, value)
```

```{r geomorph_lm, eval=FALSE, cache=TRUE}
...
```{r median_tbl, echo=F, results="asis", anchor="table", fig.cap="Median table"}
med_wide <- meds %>% spread(key=family, value=value)
med_wide %>% knitr::kable(digits = 2, caption = "Median table")
```

```{r mad_tbl, echo=F, results="asis", anchor="table", fig.cap="MAD table"}
mad_wide <- mads %>% spread(key=family, value=value)
mad_wide %>% knitr::kable(digits = 2, caption="MAD table")
```


```{r turker_v_expert, anchor="figure"} 
ggplot(all_stats, aes(median, mad, label=mark)) + geom_text() + labs(x="Accuracy", y="Precision")
```
...
```{r cv_table, echo=F, results="asis", anchor="table", message=FALSE}
...
CV_sum <- left_join(CV_sum, CV_sum2)

# format for display
CV_sum %>% transmute(`Family`=family, `LDA`=error, `QDA`=qda_error) %>% knitr::kable(digits=3, caption="Misprediction rate of linear discriminant analysis (LDA) and quadratic discriminant analysis (QDA) with 10-fold cross validation for each fish image.")
```
...
real_plot_tangent_space <- function(coords, axis1="PC1", axis2="PC2") {
  ...
  pcdata <- pc.res$x
	...
	res$id <- rownames(res)
	res2 <- res %>% separate(id, into=c("family", "role","sha"), sep="_") %>% tbl_df()
	...
	p <- ggplot(res2, aes_string(axis1, axis2, group="family")) + geom_point(aes(col=family, shape=role)) + geom_point(aes(x, y), data=consensus, alpha=0.5, size=5, col="black") + geom_point(aes(x, y, col=family, shape=role), data=consensus) + stat_ellipse(aes(color=family), linetype=2, type="norm", level=0.95) + theme_minimal() + theme(legend.position="none")
...
results <- read_tsv("fishturk.results") %>% filter(Answer.marks != "") %>% mutate(tip=str_match(annotation, ".*/(.*)/([A-Za-z]+_[A-Za-z]+)")[, 3], family=str_match(annotation, ".*/(.*)/([A-Za-z]+_[A-Za-z]+)")[, 2], Answer.marks=str_replace_all(Answer.marks, '""', '"'))
...
res <- plotTangentSpace(full_coords, groups=groups, axis1=1, axis2=2, verbose=T, warpgrids=F)$pc.scores
...
```{r timing_plot, echo=FALSE, fig.cap="Line plot showing time to receive results for any given image (x axis) and the total fraction of the data set received (y axis). Landmarks were first received 6 minutes after creation of the Amazon MTurk task, and at least one replicate was received for every image at the 80 minute mark."}
# median time (in minutes) it takes an expert to measure these fishes
timings <- (no_st %>% group_by(role) %>% summarise(duration=median(duration))) %>% mutate(total_time=duration * nrow(res2))

morpho_per_image <- timings[1, 2] %>% as.numeric() %>% dminutes() %>% round(1)

# data collection time plot
results %>% transmute(hitid, start=parse_date_time(creationtime, "bdHMSY"), end=parse_date_time(assignmentsubmittime, "bdHMSY"), diff = end - start) %>% group_by(hitid) %>% summarise(time=min(diff)) %>% arrange(time) %>% mutate(seq = 1:n(), frac = seq / n()) %>% ggplot(aes(time / eminutes(), frac)) + geom_line() + labs(x="Time taken (in minutes) to receive data for one unique replicate", y="Fraction of image set complete") + theme_minimal() + xlim(0, NA)

total_turker_time <- results %>% transmute(hitid, start=parse_date_time(creationtime, "bdHMSY"), end=parse_date_time(assignmentsubmittime, "bdHMSY"), diff = end - start) %>% summarise(max_time = max(diff)) %>% as.numeric %>% dminutes %>% round(1)
```

Using a median-expert time of `r as.character(morpho_per_image)` per image, we estimate that a single morphologist would take `r as.character(morpho_per_image * nrow(res2))` to landmark all `r nrow(res2)` images. At 5x replication, this would take `r as.character(morpho_per_image * nrow(res2) * 5)`. By comparison, turkers took a total of `r as.character(total_turker_time)` to complete all images at 5x replication.
...
XXX: Best practices for turking
...
download.file("https://www.dropbox.com/s/ehwm3c945k7rh0n/library.bib?dl=0", destfile="library.bib", method="curl", extra="-L")
```
