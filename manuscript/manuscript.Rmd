...
```{r turk_morph_procrustes_space, echo=FALSE, fig.cap="Testing my figure", anchor="figure"}
plot_tangent_space(coords, "PC1", "PC2")
```
...
fish_families %>% group_by(worker, tip) %>% summarise(time=min(time_taken)) %>% arrange(time) %>% mutate(seq = 1:n(), frac = seq / n()) %>% ggplot(aes(dseconds(time) / eminutes(), frac)) + geom_line() + labs(x="Time taken (in minutes) to receive data for one unique replicate", y="Fraction of image set complete") + theme_minimal() + xlim(0, NA)
...
```{r turker_morph_procrustes_other_pc23, echo=FALSE, message=FALSE, warning=FALSE, fig.cap="PC2 and PC3 for turker / morphologist Procrustes space"}
plot_tangent_space(coords, "PC2", "PC3")
```
...
plot_tangent_space(coords, "PC3", "PC4")
```

```{r turker_morph_procrustes_other_pc45, echo=FALSE, message=FALSE, warning=FALSE, fig.cap="PC4 and PC5 for turker / morphologist Procrustes space"}
plot_tangent_space(coords, "PC4", "PC5")
```
