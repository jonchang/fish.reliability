...
Turkers in aggregate required `r as.character(total_turker_time)` to complete all `r nrow(distinct(fish_families, tip))` images at 5x replication `r figr("timing_plot")`. We estimate that a single expert would need `r as.character(morpho_per_image * nrow(distinct(fish_families, tip)))` to complete all images at 1x replication, extrapolated from a median expert time per image of `r as.character(morpho_per_image)`. At 5x replication, our projected expert would instead need `r as.character(morpho_per_image * nrow(fish_families))`.
...
```{r bamm_plots, message=FALSE, echo=FALSE, warning=FALSE, results="hide", fig.show="hold", anchor="figure", fig.cap="Rates of shape evolution for PC1 across seven families of fishes. (a) Phylorate plots color branch lengths by rates of shape evolution, where warmer colors indicate faster rates of evolution. Significant rate shift events (pp > 0.95) are indicated on the phylorate plot as a red circle on the corresponding branch. (b) Median log rates of shape evolution through time, where black lines indicate the background rate and red lines indicate the rate of phenotypic evolution in a clade experiencing a significant shift in rate, corresponding to red circles in (a).", fig.width = 6, fig.height = 3, out.height="3in", out.width="6in"}
...
layout_mat <- matrix(c(
  8, 10, 12, 14, 16, 18, 20, 22, # shape phylorates
  9, 11, 13, 15, 17, 19, 21, 23, # shape rate through time
  0, 1, 2, 3, 4, 5, 6, 7
), nrow = 3, byrow = TRUE)
...
Using the broken-stick method of determining a PCA stopping point, we analyzed PC 1 through PC `r stopping_pc`. We project per-species consensus shapes into Procrustes space (Figure `r figr("sevenfamily_tangent_plot")`, Supplemental Figure S5). The BAMMtools analysis uncovered heterogeneity in the rate of body shape evolution in each family (Figure `r figr("bamm_plots")`). Significant shifts in the rate of shape evolution were detected within two families: Labridae and Pomacentridae. Two significant shifts in shape evolution rate occur in the wrasses (Labridae). The first rate shift occurs deep in the tree, corresponding to the lineage containing the labrine, scarine, and cheiline tribes. The other shift is nested within that group, in *Sparisoma*. One shift in the rate of shape evolution occurs in the damelfishes (Pomacentridae) in the genus *Amphiprion*.
