...
```{r sum_table}
anon_csv <- read_csv("anon.csv")
no_st <- anon_csv %>% filter(role != "student") %>% mutate(role=factor(role), mark=factor(mark), Family = str_replace(fn, "(.{3}).*", "\\1")) %>% select(-fn)
...
CV <- subsamp %>% group_by(fn) %>% do(estimator = errorest(role ~ mark:x + mark:y, data=., model=qda, estimator="cv", est.para = control.errorest(k=10), predict=ip.lda))

CV_sum2 <- CV %>% summarise(fn=fn, qda_error=estimator[["error"]]) %>% separate("fn", "fn", sep="_", extra="drop")

CV_sum <- left_join(CV_sum, CV_sum2)

# format for display
CV_sum %>% transmute(`Family`=fn, `LDA`=error, `QDA`=qda_error) %>% knitr::kable(digits=3, caption="Misprediction rate of linear discriminant analysis (LDA) and quadratic discriminant analysis (QDA) with 10-fold cross validation for each fish image.")
```
