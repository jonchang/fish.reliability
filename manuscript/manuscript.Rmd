...
meds <- pcaed %>% group_by(family, role, mark) %>%
  summarise(med=median(PC1)) %>%
  ungroup() %>%
  spread(role, med) %>%
  transmute(family, mark, value = (abs(turker - morphologist)), stat="median")
...
all_stats <- bind_rows(meds, mads) %>% spread(stat, value) %>% group_by(family) %>% mutate(lowerq=quantile(median)[2], upperq=quantile(median)[4], iqr=IQR(median), outlier = median > iqr * 2 + upperq) %>% select(-lowerq, -upperq, -iqr)
...
all_stats %<>% left_join(full_family) %>% mutate(outlier_label = ifelse(outlier, as.character(mark), NA))
```


```{r turker_v_expert, fig.cap="Test figure caption", anchor="figure"}
ggplot(all_stats, aes(median, mad, color=outlier)) + geom_point() + labs(x="Accuracy", y="Precision") + facet_wrap(~full_family) + geom_dl(aes(label=outlier_label), size=1, list("top.bumptwice", dl.trans(y=y+0.1), cex=0.75)) + scale_color_manual(values=c("black", "red")) + ylim(-max(abs(all_stats$mad) + 0.25), max(abs(all_stats$mad) + 0.25)) + theme("legend.position" = "none")
```



```{r geomorph_lm, eval=FALSE, echo=FALSE, cache=TRUE}
...
```{r median_tbl, eval=F, echo=F, results="asis", anchor="table", fig.cap="Median table"}
med_wide <- meds %>% spread(key=family, value=value)
med_wide %>% knitr::kable(digits = 2, caption = "Median table")
```

```{r mad_tbl, eval=F, echo=F, results="asis", anchor="table", fig.cap="MAD table"}
...
```{r timing_plot, echo=FALSE, fig.cap="Line plot showing time to receive results for any given image (x axis) and the total fraction of the data set received (y axis). Landmarks were first received `r timing_data[[1, 'time']] %>% dminutes()` after creation of the Amazon MTurk task, and at least one replicate was received for every image at the 80 minute mark."}
