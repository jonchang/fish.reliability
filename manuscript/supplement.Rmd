...
lm_result <- advanced.procD.lm(res$coords ~ family*role*sha1mac, ~family*sha1mac, .parallel.cores = 24, .parallel.cluster = parallel::makeForkCluster(24))
lm_p <- lm_result$P[2]
lm_Z <- lm_result$Z[2]
lm_F <- lm_result$F[2]
...
We also used a non-parametric MANOVA with RRPP to determine whether turkers were less consistent among individual replicates as compared to experts. The non-parametric MANOVA with RRPP failed to detect a significant difference between turker and expert shapes ($p = `r lm_p`$, $Z = `r lm_Z`$, $F=`r lm_F`$). 
